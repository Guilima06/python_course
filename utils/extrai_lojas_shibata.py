...
file_path = '/home/guilherme/Dropbox/HTML/Lojas/Shibata.html'
...
stores = soup.find_all('div', {'class': 'store-card'})
# print(stores)
...
for store in stores:
    ...
    name_element = store.find('h2')
    if name_element is not None:
        # Extraindo as informações
        name = name_element.text.strip().title()
        address = store.find('address').text.strip().replace('\n                    ','')
        dados_loja = {
            'Nome': name,
            'Endereço': address,
        }
...
df.to_excel('/home/guilherme/Dropbox/HTML/Lojas/Storesshibata.xlsx', index=False)
