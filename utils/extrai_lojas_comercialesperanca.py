...
file_path = '/home/guilherme/Dropbox/HTML/Lojas/comercialesperanca.html'
...
stores = soup.find_all('div', {'class': 'infobox__body'})
print(len(stores))
...
for store in stores:
    ...
    name_element = store.find('div', {'class': 'store-location'})
    if name_element is not None:
        # Extraindo as informações
        name = name_element.text.strip().title()
        address = store.find('div', {'class': 'store-address'}).text.strip()
        dados_loja = {
            'Nome': name,
            'Endereço': address,
        }
...
df.to_excel('/home/guilherme/Dropbox/HTML/Lojas/Storesesperanca.xlsx', index=False)
